...
var Admin = function (args) {
	...
	body.setAttribute(
		"class",
		(body.getAttribute("class") || "") + " showcase",
	);
...
Admin.prototype.setupSlides = function (slides) {
	...
	while (count < l) {
		sideBarContentUl.addMessage(
			count +
				". <strong>" +
				this.getSlideName(slides[count]) +
				"</strong>",
			...
			var value,
				...
				update = function (setValue, single) {
					...
					if (typeof setValue === "number") {
						...
						obj[valueName] =
							(value - min) * outputFactor + outputMin;
...
Admin.prototype.setupBasicControls = function () {
	...
	if (this.showcase || this.admin) {
		createButton({
			...
			args: false,
			className: "important slideControl narrow newrow",
		});
		createButton({
			text: "▶︎︎",
			functionName: "nextSlide",
			args: true,
			className: "important slideControl narrow",
		});
	}
