...
BBObjProto.calcRotation = function( rotate ) {
	...
	if( realRotation > 180 ) { realRotation -= 360; }
	else if ( realRotation < -180 ) { realRotation += 360; }
	...
	return {
		...
		position : ( sin + cos ) / ( Math.sin( Math.PI * .25 ) * 2 ),
		sin : sin,
		cos : cos,
		rotate: rotate,
	...
	( args.roundTop || args.roundBottom ) && this.list.push(
		{
			...
			list:[
				...
				args.roundBottom && { name:"Dot", fX:true, fY:true, clear:true },
			]
		}
	);
