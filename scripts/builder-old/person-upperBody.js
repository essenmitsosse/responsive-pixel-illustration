...
Builder.prototype.UpperBody = function ( args ) {
	...
	this.chestWide = this.IF(.05);
	this.chestSX = this.chestWide && this.R(1,1.3) || 1;
	this.chestSY = this.chestWide && this.R(.3,.8) || 1;
	this.chestStartSY = this.chestWide && this.R(.1,.5) || 0;

	this.topless = args.topless = args.animal || this.IF(.02);

	this.waist = !this.chestWide && this.IF(.05);
	this.breast = this.IF(.05);

	this.hanky = !this.topless && this.IF(.02);
	...
	this.shirtColor = args.shirtColor = ( this.IF(.5) ? args.secondColor : args.clothColor ).copy({ brSet: shirtColor === 4 ? 3 : shirtColor + 1, max:4 });

	// Assets
	this.arm = new this.basic.Arm( args );
	this.IF(.05) && ( this.cape = new this.basic.Cape( args ) );
	this.topless && this.IF(.8) && ( this.nipples = new this.basic.Nipples( args ) );
	
	!args.animal && (
		this.IF(.07) && ( this.suspenders = new this.basic.Suspenders( args ) ),
		this.IF(.02) && ( this.strap = new this.basic.Strap( args ) )
	);

	!this.topless && (
		!this.breast && !this.chestWide && this.IF(.05) && ( this.stripes = new this.basic.Stripes( args ) ),
		this.IF( .4 ) && ( this.collar = new ( this.IF() ? this.basic.Cleavage : this.basic.Collar )( args ) ),
		this.IF( .3 ) && ( this.buttons = new this.basic.Buttons( args, this.clothColor ) )
	);

	this.IF( .1 ) && ( this.logo = new this.basic.Logo( args ) );
...
Builder.prototype.UpperBody.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		...
		this.vL[ "chestSX"+nr ] = { r:this.chestSX, useSize:"upperBodySX"+nr, min:1, max:["upperBodySX"+nr,1] };
	...
	return {
		...
		list:[
			...
			this.hanky && !backView && ( sideView || !args.right ) && {
...
Builder.prototype.Stripes = function ( args ) {
	// Form & Sizes
	this.gap = this.R(.05,.2);
	this.strip = this.R(.05,.2);
	this.horizontal = this.IF(.5);
	this.randomDots = this.IF(.05);
	this.doted = !this.randomDots && this.IF(.1);
	this.doted && (
		this.dotGap = this.R(.05,.2),
		this.dotStrip = this.R(.05,.2)
	);

	// Colors
	this.stripColor = ( this.IF(.5) ? args.secondColor : args.clothColor ).copy({ brSet:args.clothColor.getBr()-1 })
...
Builder.prototype.Stripes.prototype.draw = function ( args, z ) {
	...
	return ( sideView || !args.right ) && {
		...
		this.randomDots ? 
			[
				{ use:"shirt"+nr, chance:.5, sX:{r:this.gap, useSize:"upperBodySY"+nr}, sY:{r:this.strip, useSize:"upperBodySY"+nr}, mask:true },
				{ save:"shirt"+nr }
			]
			: this.doted && [
					{
...
Builder.prototype.Buttons = function ( args, color ) {
	// Form & Sizes
	this.buttonSX = this.R(-.2,.2);
	this.zipper = this.IF(.1);
	this.buttonSY = !this.zipper && this.R(-.1,.1);
	this.buttonGapSY = !this.zipper && this.R(-.1,.1);
...
Builder.prototype.Suspenders = function ( args ) {
	// Form & Sizes
	this.strapSX = this.R(-.8,.5);
	this.strapX = this.R(.5,1);

	this.detail = this.IF(.5);

	// Colors
	this.strapColor = ( this.IF(.5) ? args.firstColor : args.secondColor ).copy( { brContrast:-1 } );
	this.detail && ( this.detailColor = args.clothColor.copy( { brContrast:this.IF(.5) ? 1 : -1 } ) );
...
Builder.prototype.Suspenders.prototype.draw = function ( args, z ) {
	...
	return {
		...
		list:[
			{
				sX:{ r:this.strapSX * ( sideView ? .5 : 1 ), useSize:"trapSX"+nr, min:1, save:"strapSX"+nr },
				x:{ r:this.strapX * ( sideView ? .5 : 1 ), useSize:"trapSX"+nr, max:["trapSX"+nr,this.sub("strapSX"+nr)], min:{a:0} },
				fX:true,
				list:detail
			},
			sideView && {
				sX:{ r:this.strapSX * ( sideView ? .5 : 1 ), useSize:"trapSX"+nr, min:1, save:"strapSX"+nr },
				x:{ r:this.strapX * ( sideView ? .5 : 1 ), useSize:"trapSX"+nr, max:["trapSX"+nr,this.sub("strapSX"+nr)] },
				list:detail
			}
		]
	};
...
Builder.prototype.Collar = function ( args, cleavage, sleeveless ) {
	// Form & Sizes
	this.collarSY = this.R(.1,.5);
	this.open = this.IF(.2);
	this.tie = this.IF();
	this.fullTie = this.open || this.IF();
	this.scarf = !this.tie && this.IF(.05);
	...
	// Assets
	this.open && this.IF(.3) && ( this.buttons = new this.basic.Buttons( args, this.shirtColor ) )
...
Builder.prototype.Collar.prototype.draw = function ( args, z ) {
	...
	return {
		z:10,
		color:this.shirtColor.get(),
		sX:this.scarf ? { a:"headSX"+nr, max:"chestSX"+nr } : ["neckSX"+nr,sideView ? 2 : 1],
		...
		list:[
			...
			this.buttons && {
				...
				sY:"upperBodySY"+nr,
				list:[
					this.buttons.draw( args, z ),
				]
			},			

			!args.backView && this.tie && {
				sX:{ r:.25, min:1 },
...
Builder.prototype.Cleavage = function ( args, cleavage, sleeveless ) {
	// Form & Sizes
	this.sleeveless = !args.sleeves && this.IF(.5);
	this.strapSX = this.sleeveless && this.R(-1,0);
	this.strapSY = this.sleeveless && this.R(.05,.3);

	this.cleavage = !this.sleeveless || this.IF(.5);
	this.cleavageSX = this.cleavage ? this.cleavage && this.R(1,2) : 1;
	this.cleavageSY = this.cleavage && this.R(.05,.3);
...
Builder.prototype.Cleavage.prototype.draw = function ( args, z ) {
	...
	if( args.calc ) {
		this.vL[ "cleavageSX"+nr ] = {r:this.cleavageSX, useSize:"neckSX"+nr, max:["chestSX"+nr,-2] };
		this.vL[ "cleavageX"+nr ] = sideView ? 
			[ this.mult(.5,"chestSX"+nr ), this.mult( -.5, "cleavageSX"+nr ), this.sub("collarSX"+nr) ] 
			: [ "chestSX"+nr, this.mult( -1, "cleavageSX"+nr ) ] 
		this.sleeveless && (
			sideView && (
				this.vL[ "cleavageRightX"+nr ] = [ "chestSX"+nr, this.sub( "cleavageX"+nr ), this.sub( "cleavageSX"+nr ) ]
			),
...
Builder.prototype.Nipples = function ( args ) {
	// Form & Sizes
	this.nippleSize = this.R(.01,.3);
	this.nipplePos = this.R(.15,.4);
...
Builder.prototype.Cape = function ( args ) {
	// Form & Sizes
	this.capeFrontSY = this.R(.1,.8);
	this.capeSY = this.R(.3,1);
...
Builder.prototype.Cape.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		...
		this.vL[ "capeSY"+nr ] = { r:this.capeSY, useSize:"fullBodySY"+nr, max:["fullBodySY"+nr,-1], min:"capeFrontSY"+nr };
	}
...
Builder.prototype.Strap = function ( args ) {
	// Form & Sizes
	this.thickness = this.R(.01,.05);

	// Color
	this.strapColor = args.beltColor || ( args.beltColor = ( this.IF(.5) ? args.firstColor : args.secondColor ).copy( { brContrast:this.IF() ? 2 : -2 } ) );
...
Builder.prototype.Strap.prototype.draw = function ( args, z ) {
	...
	return ( sideView || args.right ) && {
		...
		rX:sideView && !args.right,
		list:[
			{ tY:true, tX:true, clear:true, sY:"strapTickness"+nr, mX:this.sub("strapTickness"+nr) },
			{
				z:z,
				weight:"strapTickness"+nr,
				color:this.strapColor.get(),
				points:[
					{ fX:true, fY:true, x:this.mult(.5,"strapTickness"+nr) },
					{ fX:true, x:{r:sideView ? 1 : 2, useSize:"personSX"+nr, add:[this.mult(-.5,"strapTickness"+nr),-2] } }
				]
			}
		]
	};
