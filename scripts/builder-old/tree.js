...
Builder.prototype.TreeFamily = function ( args ) {
	this.chance = this.R();
	// Forms & Sizes
	this.minFoliagePos = ( this.R(0,1) + .4 ) * .5;

		// branches
	this.branchCount = Math.floor( this.R(0,1) * 3 + 3 );
	this.thinner = .8 / this.branchCount;
	this.shorter = this.R(0,1) * .8;
	this.horFactor = this.R(0,1) * .6 + .4;

	this.fruit = this.IF();
	this.fruitChance = this.fruit && this.R(0,1) * .02,
	this.fruitSize = this.fruit && this.GR(2,4);

	this.leaveX = this.GR(1,4);
	this.leaveY = this.GR(1,4);

	this.leaveDetail = this.IF(.8);
	...
	this.fooliageCoverage = this.IF(.3) && this.R(0,1) * .5 + 1;
	...
	this.trunkColor = args.groundColor ? 
		( args.skyColor && this.IF(.2) ? args.skyColor : args.groundColor ).copy({ brSet:Math.floor( this.GR( 1, 4 ) ) })
	...
	this.foliageColor = args.skyColor ?
		( args.groundColor && this.IF(.8) ? args.skyColor : args.groundColor ).copy({ brContrast:this.IF() ? -2 : 2 })
	...
	this.fruitColor = this.fruit && this.foliageColor.copy( { nextColor:this.IF(.8), brContrast:2 } );
};
...
// TREE
Builder.prototype.Tree = function ( args, z ) {
	if( !args ) { args = args || {} };
	...
	// Sizes & Forms
	this.crookedY = ( this.crooked && this.R(.2,.9) ) || undefined;
	this.crookedSX = this.crooked && this.R(1,4);
	this.reflectCrookedTrunk = this.IF(.5);

	this.foliageSX = this.R(.2,.6);
	this.foliageSY = this.R(0,.2) * ( 1 - this.minFoliagePos ) + .2;

	this.trunkSX = this.R(.02,.1);

	this.leaveChange = this.R(0,1);

	this.random = [ this.R(0,1), this.R(0,1), this.R(0,1), this.R(0,1), this.R(0,1), this.R(0,1), this.R(0,1) ];
...
Builder.prototype.Tree.prototype.draw = function ( args, z, size ) {
	...
	this.vL[ "foliageSX"+code ] = {r:this.foliageSX, useSize:"treeSqu"+code, min:1 };
	this.vL[ "foliageSY"+code ] = {r:this.foliageSY, useSize:"foliageSX"+code, min:1 };
	...
	return [
		...
		// Detail Back
		this.leaveDetail && !this.fooliageCoverage && {
			color: this.foliageColorDetail.get(),
			seed:this.nr,
			chance: .02,
		...
		{
			cX:true, 
			sX:{r:this.trunkSX, useSize:"treeSqu"+code, min:1, save:"trunkSX"+code }, 
			sY:{ r:this.crookedY, useSize:"treeSqu"+code, save:"topTrunk"+code }, 
			color: this.trunkColor.get(),
			z:this.zInd, 
			list:[
				{ list: this.addBranches( true, true, this.branchCount, 1 ) },
				this.crooked ? 
					{ save:"trunk"+code, rX:this.reflectCrookedTrunk, list:[
						{ sY:"trunkSX"+code, fY:true, sX:{ r:this.crookedSX, useSize:"trunkSX"+code}, list:[
							{ },
							{ fX:true, sX:"trunkSX"+code, sY:[ "treeSqu"+code, this.sub("topTrunk"+code) ], fY:true, tY:true }
						] },
					]}
					: { save:"trunk"+code },
			] 
...
Builder.prototype.Tree.prototype.addBranches = function ( hor, parentLeft, count, level ) {
	var list = [],
		i = Math.floor( count ),
		step = ( 1 / i ) * this.minFoliagePos, 
		left = true,
		thisZ = ( level === 1 || ( this.getRandom() < .2 ) ? "Front" : "Back" ),
		strip = { stripes:{ random:{r:-.5}, strip:this.leaveX, change:{r:this.leaveChange}, seed:this.nr + count + level, cut:true } },
		code = this.code,
		leaves = [
			// Shadow of Leaves
			...
			// Top of Leaves
			{ save:"leaves"+thisZ+code, tY:true, sY:{r:.1, min:1}, list:[
				{ fY:true, stripes:{ strip:this.leaveX, random:{r:-1}, change:{r:this.leaveChange*5}, cut:true } }
			]},
		];
	...
	list.push( { list:[
		...
		( level < 3 ) && {
			...
			rotate: !hor ? 90 : 0,
			fY:!hor || ( level >= 3 && hor ),
			y:this.mult( -.3, ( !hor ? "foliageSX" : "foliageSY" ) + code ), 
			rY:!parentLeft,
			list: [
				{ sX:{r:.5, a:this.leaveX}, list:leaves },
				{ sX:{r:.5, a:this.leaveX}, fX:true, rX:true, list:leaves },
			]
		}
	] } );
	...
	if( i > 0 && level <= 3 ) {
		...
		while( i -- ) {
			...
			list.push({
				sX:{ r:this.getRandom() * this.shorter * ( !hor ? this.horFactor : 1 ) * ( level === 1 ? this.minFoliagePos + .2 : 1 ), otherDim:true, min:1 }, 
			...
			list.push( { 
				...
				fX:left,
				tX:true,
				rY: ( ( hor && ( ( !parentLeft && !left ) || ( parentLeft && left ) ) ) || !hor && ( ( parentLeft && !left ) || ( !parentLeft && left ) ) ),
				rotate: 90,
...
Builder.prototype.Tree.prototype.getRandom = function() {
	this.randomCount += 1;
	if( this.randomCount > this.random.length ) { this.randomCount = 0; }

	return this.random[ this.randomCount ];
}

// FORREST
Builder.prototype.Forrest = function ( args ) {
	var i = this.treeKindsCount = this.IF(.8) ? 1 : this.IF(.8) ? 2 : 3,
		trees,
		family,
		count;
