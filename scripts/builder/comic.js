...
Builder.prototype.Comic = function ( init ) {
	var list = [],
		...
		outerBorderColor = this.IF( 0.9) && borderColor.copy({ brContrast:this.IF( 0.8) ? 5 : 3, dontChange:this.IF( 0.5) }),
	...
	this.vL["panelSY"] = { 
		add:[
			{ add:[ "imgSY", this.mult(-1,"_panelOvershot2IF") ], min:this.mult( 0.5,"panelRest2SY")  },
...
Builder.prototype.Panel = function ( args ) {
	...
	if( this.lightnessSwitched ) {
		...
		this.newLightness = args.newLightness = args.newLightness || this.GR(1,3) * ( this.IF( 0.7) ? -1 : 1 );
	...
	// Colors
	this.skyColor = args.skyColor = args.borderColor.copy({nextColor:this.IF( 0.1), brSet:this.sun ? 4 : 5 });
	if( this.sun ) { 
...
Builder.prototype.Panel.prototype.draw = function ( args ) {
	var nr = args.i,
		...
		actionBackground = this.IF( 0.01 + ( closeUp ? 0.3 : 0 ) + ( wideShot ? 0.05 : 0 ) ) && 
	...
	this.vL["thingS"+nr] = this.mult(closeUp ? 2 : wideShot ? 0.5 : superWideShot ? 0.2 : 1,"panelSqu");
	this.vL["horizont"+nr] = {
	...
	actors = {
		...
		x:{r:this.IF( 0.5) ? 0.2 : -.2, useSize:"thingS"+nr},
...
Builder.prototype.Actor.prototype.draw = function ( args, z, size ) {
	...
	args.arm = { 
		left: args.shoulder.left > 0.55  ? this.R( 0.5 ) : this.IF( 0.7) ? this.R(-.75, 0.75) : this.R(-.25, 0.25), 
		right: args.shoulder.right > 0.55  ? this.R( 0.5 ) : this.IF( 0.7) ? this.R(-.75, 0.75) : this.R(-.25, 0.25) 
	};
