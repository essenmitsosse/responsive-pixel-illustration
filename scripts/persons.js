	...
	var help = helper,
		...
		Person = function () {
			var i = (personC += 1),
				...
				bodySY = Math.random() * 0.6 + 0.3,
				armSX = Math.random() * 0.4 - 0.2,
				armSY = Math.random(),
				armWid = Math.random(),
				legsSY = Math.random(),
				// legSX = Math.random(),
				crotchSY = Math.random() * 0.5,
				headMX = Math.random() * 0.5,
				headSideSX = Math.random() * 0.5 + 0.4,
				neckSX = Math.random(),
				neckSY = Math.random() * 0.3,
				neckMX = Math.random() * 0.4,
				faceMX = Math.random() * 0.3,
				faceSY = Math.random() * 0.8,
				faceY = Math.random() * 0.9,
				faceSideSX = Math.random() * 0.8,
			...
			joinVariableList["personSX" + i] = margin(
				"personS",
				"personMX" + i,
			);
			...
			joinVariableList["armSX" + i] = {
				r: armSX,
				min: 1,
				useSize: "personSX" + i,
			};
			joinVariableList["armSY" + i] = {
				r: armSY,
				useSize: "bodySY" + i,
				min: 3,
			};
			(joinVariableList["armWid" + i] = {
				r: armWid,
				min: 1,
				max: "headSX",
				save: "armWid",
			}),
				(joinVariableList["legSY" + i] = {
					r: legsSY,
					useSize: "bodySY" + i,
					max: ["bodySY" + i, -1],
				});
			...
			joinVariableList["headSX" + i] = margin(
				"torsoSX" + i,
				"headMX" + i,
			);
			joinVariableList["headSideSX" + i] = {
				r: headSideSX,
				useSize: "torsoSX" + i,
				min: 2,
			};
			joinVariableList["neckHeadSY" + i] = [
				"personSY" + i,
				sub("bodySY" + i),
			];
			joinVariableList["neckSY" + i] = {
				r: neckSY,
				useSize: "neckHeadSY" + i,
			};
			joinVariableList["headSY" + i] = [
				"neckHeadSY" + i,
				sub("neckSY" + i),
			];
			joinVariableList["neckMX" + i] = {
				r: neckMX,
				useSize: "headSideSX" + i,
			};
			joinVariableList["neckSX" + i] = {
				add: ["headSX" + i, mult(-2, "neckMX" + i)],
				max: { a: "torsoSX" + i, max: "headSideSX" + i },
				min: 1,
			};

			joinVariableList["faceMX" + i] = {
				r: faceMX,
				useSize: "headSX" + i,
				max: [mult(0.5, "headSX" + i), -1],
			};
			joinVariableList["faceSX" + i] = [
				margin("headSX" + i, "faceMX" + i),
			];
			joinVariableList["faceSY" + i] = {
				r: faceSY,
				useSize: "headSY" + i,
				min: 3,
				max: "headSY" + i,
			};
			joinVariableList["faceSideSX" + i] = {
				r: faceSideSX,
				useSize: "headSideSX" + i,
				min: 1,
			};
			...
			joinVariableList["hairFrontSY" + i] = mult(
				hairFrontSY,
				"faceY" + i,
			);
			joinVariableList["headBackSideSX" + i] = [
				"headSideSX" + i,
				sub("faceSideSX" + i),
			];
			joinVariableList["hairBackSideSX" + i] = mult(
				hairBackSide,
				"headBackSideSX" + i,
			);
						...
						list: [
							{
								color: skinC,
								sY: "neckHeadSY" + i,
								sX: headSide ? "headSideSX" + i : "headSX" + i,
								cX: true,
								list: [
			...
			while (i < max) {
				...
				if (!isCol) {
					...
					args.eye = {
						lookVert:
							eyeLookVert[
								Math.floor(Math.random() * eyeLookVert.length)
							],
						lookHor:
							eyeLookHor[
								Math.floor(Math.random() * eyeLookHor.length)
							],
						lids: eyeLids[
							Math.floor(Math.random() * eyeLids.length)
						],
						brow: eyeBrow[
							Math.floor(Math.random() * eyeBrow.length)
						],
					};

					args.mouth = {
						height: mouthHeight[
							Math.floor(Math.random() * mouthHeight.length)
						],
						width:
							mouthWid[
								Math.floor(Math.random() * mouthWid.length)
							] && "narrowSide",
						form: mouthForm[
							Math.floor(Math.random() * mouthForm.length)
						],
						teeth: teethPos[
							Math.floor(Math.random() * teethPos.length)
						],
						smirk: Math.random() < 0.08,
					};

					args.shoulder = {
						left: Math.random() > 0.2 && Math.pow(Math.random(), 3),
						right:
							Math.random() > 0.2 && Math.pow(Math.random(), 3),
					};
					...
					args.leg = {};
					args.leg[Math.floor() < 0.5 ? "right" : "left"] =
						legPos[Math.floor(Math.random() * legPos.length)];

					args.hatDown = Math.random() < 0.02;
