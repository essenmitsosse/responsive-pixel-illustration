...
var renderer = function( args, init, createSlider ){
	var helper = window.helper,
		...
		smallerSide = linkListPush( { add: [ width, -count ], max: [ height, -count ] } ),
		biggerSide = linkListPush( { add: [ width, -count ], min: [ height, -count ] } ),
		...
		whiteSX = linkListPush( { add: [ innerSingleSY, { r: -2, useSize: stripSX } ] } ),
		...
		sizes = ( function ( count ) {
			...
			while ( i < count ) {
				obj[ "s" + i ] = linkListPush( { r:0, useSize: stripMaxSX, min: singleSY } );
				i += 1;
			}
	...
	if( createSlider ) {
		createSlider.slider( { niceName: "Steifen Master", valueName: "master", defaultValue: 1, input: { min: 0, max: 1, step: 0.01 } } );
	}
	...
	while ( i < count ) {
		...
		if( createSlider ) {
			createSlider.slider( { niceName: "Streifen " + i + " Breite", valueName: "stripSX" + i, defaultValue: 0, input: { min: 0, max: 1, step: 0.01 } } );
		}
		i += 1;
	}
