...
TableComic.prototype.Panel.prototype.draw = function (args) {
	var info = args.info || {},
		zoomSX = (this.zoomSX = this.pushLinkList({
			r: 1,
			useSize: this.minSX,
		})),
		zoomSY = (this.zoomSY = this.pushLinkList({
			r: 1,
			useSize: this.minSY,
		})),
		restSX = this.pushLinkList({
			add: [args.sX, { r: -1, useSize: zoomSX }],
		}),
		restSY = this.pushLinkList({
			add: [args.sY, { r: -1, useSize: zoomSY }],
		}),
		panXrel = (this.panXrel = this.pushLinkList({
			r: 0.5,
			useSize: restSX,
		})),
		panYrel = (this.panYrel = this.pushLinkList({
			r: 0.5,
			useSize: restSY,
		})),
...
TableComic.prototype.Panel.prototype.faceDraw = function (args) {
	var actor = args.info.actor.what,
		zoomX = (this.zoomX = this.pushLinkList({
			r: 1,
			useSize: this.minPanelSX,
		})),
		zoomY = (this.zoomY = this.pushLinkList({
			r: 1,
