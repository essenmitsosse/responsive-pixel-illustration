...
TableComic.prototype.Arm.prototype.draw = function ArmDraw ( args ) {
	var info = args.info,
		...
		armS = this.armS = this.pushLinkList( { r: 0.08, useSize: armSY, min:1 } ),
		upperArmS = this.pushLinkList( { r: 0.1, useSize: armSY, min:1 } ),
		armSHalf = this.armSHalf = this.pushLinkList( { r: 0.5, useSize: upperArmS, a:-1 } ),

		handS = this.handS = this.pushLinkList( { r: 0.1, useSize: armSY, min:1 } ),
		handSY = this.pushLinkList( { r: 0.14, useSize: armSY, min:1 } );

	this.right = args.right;

	this.isRotated = this.actor.isRotated;
	this.x = this.pushLinkList( { add: [
