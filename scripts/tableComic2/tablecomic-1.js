...
TableComic.prototype.getFace = function () {
	...
	return {
		...
		background: new this.basic.Background({ blank: true, backgroundColor: backgroundColor }),
		panels: [
			getPanel(),
			// getPanel(),
			// getPanel(),
...
TableComic.prototype.getStrip = function getStrip () {
	var stripInfo = new this.getStripInfo(),
		...
		currentPanel,
		finalPanels = [],
		combiner = this.getCombiner( finalPanels, stripInfo.defaultPanels, stripInfo.inventory );
	...
	return function ( panel ) {
		var panelList = panel.list || {},
			...
			copyDefaultObject = function ( object, defaultObject ) {
				...
				} else { // If Info already exists about that object
...
TableComic.prototype.getStripInfo = function () {
	...
	var actor1 = new this.basic.Actor({
		...
		table = new this.basic.Table({ 
			color: furnitureColor, 
			sX: 0.6,
			colorDetail: [ 70,90,80 ]
		}),
		chair1 = new this.basic.Chair( { color: furnitureColor, } ),
		chair2 = new this.basic.Chair( { color: furnitureColor, toLeft: true } ),
		glass = new this.basic.Glass( {
			color: [ this.rInt( 0, 255 ), this.rInt( 0, 255 ), this.rInt( 0, 255 ) ]
		} ),

		emotion1 = new this.basic.Emotion( {} ),
		emotion2 = new this.basic.Emotion( {} ),
		...
		actor1RightHandOnLap = {
			pos: { obj: actor1.body.legs, posX: 0.8, posY: 1 },
			hand: { angle: -0.5 },
		},
		actor1RightHandOnTable = {
			pos: {
				obj: table,
				posY: 1,
				ellbow: true
