...
var renderer = function(){
	var help = helper,
		...
		centerY = linkListPush( { r: 0.5, useSize: motiveSY, add: [ { r: -0.02, useSize: overshotSX } ], min: 1 } ),

		movementSY = linkListPush( { 
			add: [ { r: 0.9, useSize: motiveSY }, { r:-1, useSize: centerY }, { r: 0.03, useSize: overshotSX }, { r: -0.1, useSize: overshotSY } ] 
		} ),
		movementSX = linkListPush( { 
			add: [ { r: 0.7, useSize: motiveSquBigger }, { r: -1, useSize: overshotSY }, ], 
		...
		getBodyPartSize = function ( rel, height, min ) {
			return  linkListPush( {  r: rel, useSize: height ? movementSY : movementSX, min: min ? 1 : undefined } );
		},
		...
		shoulderSY = linkListPush( { r: shoulderSYRel, useSize: movementL, min: 1, max: { r: 0.2, useSize: movementSY } } ),

		torsoL = linkListPush( { getLength:[ 
		...
		hipSY = linkListPush( { r: hipSYRel, useSize: movementL, min: 1, max: { r:0.15, useSize: movementSY } } ),
		...
		legUpper1L = legSY,
		legLower1L = linkListPush( { add:[ legL, { r: -1, useSize: legSY } ], min: { a: 0 }, max: { r: 1.5, useSize: legUpper1L } } ),

		legUpper2L = linkListPush( { add:[ { r: 1.3, useSize: upperLeg } ], max: linkListPush( [ legSY, { r: -1, useSize: perspectiveY }, { r: -0.5, useSize: legUpperS } ] ) } ),
		legLower2L = linkListPush( { add:[ legL, { r: -0.8, useSize: legUpper2L } ], min: { a: 0 }, max: { r: 1.5, useSize: legUpper1L } } ),
		...
		headY = linkListPush( { add: [ shoulderY, { r: -1, useSize: headSY }, { r: 0.03, useSize: overshotSX, max: linkListPush( { r: 0.5, useSize: headSY } ) } ] } ),
		...
		mouthSY = linkListPush( { r: 0.5, useSize: linkListPush( [ headSY, { r:-1, useSize: eyeY }, { r: -1, useSize: eyeSY } ] ), min: 1 } ),
		mouthX = linkListPush( { a: 0 } ),
		mouthY = linkListPush( { r: 0.3, useSize: linkListPush( [ headSY, { r:-1, useSize: eyeY }, { r:-1, useSize: eyeSY }, { r:-1, useSize: mouthSY } ] ) } ),
		...
		fruitSX = linkListPush( { r: fruitSXrel, min:2, useSize: motiveSqu, add:[ { r: fruitSXBigRel, useSize: motiveSquBigger } ], max:{ r: 0.1, useSize: motiveSqu } } ),
		fruitSY = linkListPush( { r: fruitSYrel, min:3, useSize: motiveSqu, add:[ { r: fruitSYBigRel, useSize: motiveSquBigger } ], max:{ r: 0.1 * 1.8, useSize: motiveSqu } } ),

		fruitHandMaxX = linkListPush( { add: [ handSX, foreArmS, 1, { r: -0.1, useSize: overshotSY } ], min: linkListPush( { add: [ -1, { r: 0.5, useSize: handSX }, { r: -0.5, useSize: fruitSX } ] } ) } ),
		fruitHandMaxY = linkListPush( { add: [ handSY, foreArmS, 1, { r: -0.1, useSize: overshotSX } ], min: linkListPush( { add: [ -1, { r: 0.5, useSize: handSY }, { r: -0.5, useSize: fruitSY } ] } ) } ),
		
		fruitX = linkListPush( { add:[ 
			centerX, 
			{ r:-1, useSize: handSX }, 
			fruitHandMaxX 
		] } ),
		fruitY = linkListPush( { add:[ 
			centerY, 
			handSY, { r:-1, useSize: fruitSY }, 
			{ r:-1, useSize: fruitHandMaxY } 
		] } ),

		// Island
		shadowSY = linkListPush( { add: [ perspectiveY, { r: 2, useSize: legLowerS }, { r: -0.05, useSize: overshotSY } ], min: 2 } ),
		...
		teiresias = function ( reflect ) {
			...
			return { 
				...
				list: [
					...
					// Arm Back
					{ x: [ armHandX, { r:-1, useSize: foreArmS } ], y: armHandY, sX: armShoulderX, sY: armShoulderY, list: [
						{ sX: foreArmS },
						{ fY: true, sY: upperArmS }
					] },
					...
					{ 
						...
						sY: [ shoulderSY, -1 ], 
						sX:[ hipSX, hipX, { r:-1, useSize: linkListPush( [ shoulderX, shoulderSX ] ) }, -1 ], 
						list: [
							{},
							{ color: skinShadowColor, stripes: { gap: { r: 0.1}, change: { r: -0.1 } }, sX: { r: 0.5 }, mY: { r: 0.1, min:1 } },
							{ sY:1, color: skinShadowColor, fY: true, sX: { r: 0.5, add:[ { r:-0.1, useSize: overshotSY }] } },
						] 
					},
					...
					// shoulder
					{ sX: shoulderSX, sY:shoulderSY, x: shoulderX, y: shoulderY, id:"shoulder", list:[
						{},
						{ sY: { a:1, r:-0.01, useSize: overshotSY }, color: skinShadowColor, fY: true },

						// Nipples
						{ mX: { r: 0.1 }, sY: { r: 0.15, useSize: shoulderSY, min: 1 }, y: { r: 0.5, min: 1 }, color: skinShadowColor, list: [
					...
					// leg
					{ sX: legSX, sY: legSY, x: legX, y: legY,list: [
						{ sY: legUpper2L, sX:legLower2L, list: leg },
						// Leg Front
						{ tX: true, fX: true, sX: { add: [ legLower1L ], min: 1 }, x: legUpperS, list: leg },

					] },

					// Arm Front
					{ x: [ shoulderX, shoulderSX, ], y: arm2Y, sX: arm2SX, sY: arm2SY, list: [
						{ sX: upperArmS, sY: { r: 0.5 } },
						{ sX: foreArmS, sY: { r: 0.5 }, fY: true },
					] },

					// Head
					{ sX: headSX, sY: headSY, x: headX, y: headY, list: [
						{},
						// Eyes
						{ color: skinShadowColor, sX: eyesSX, sY: eyeSY, x: eyeX, y: eyeY, list: [
							{ sX: eyeSX },
							{ sX: eyeSX, fX: true }
						] },

						// Mouth
						{ sX: mouthSX, sY: mouthSY, x: mouthX, y: mouthY, color: skinShadowColor, fY: true, cX: true },
		...
		trunkObj = function ( shadowColor, hor, vert ) {
			return [
				{ fY: true, sY: hor },
				{ sX: vert },
				{ stripes: { gap: { a:3, random:{ r:4, useSize: vert } }, random: { r: 1 }, strip: vert }, fY: true, list: [
					{},
					{ color: shadowColor, sY: { r: 0.9 } },
				] },
		...
		mainImage = function () {
			return [
				...
				{ color: water, sY: waterSY, y: waterY, list: [
					{ use:"water3", },
					{ use:"water3", color: waterLight, chance: 0.05, sY:1, sX:{ a:4, random: 27 }, mask: true },
					{ save:"water3" },

					{ use:"water2", },
					{ use:"water2", color: waterLight, chance: 0.05, sY:1, sX:{ a:2, random: 9 }, mask: true },
					{ save:"water2", sY: { r: 0.5 } },

					{ use:"water", },
					{ use:"water", color: waterLight, chance: 0.05, sY:1, sX:{ a:1, random: 3 }, mask: true },
					{ save:"water", sY: { r: 0.2 } },
				] },
				...
				{ 
					...
					y: islandY,
					list: [						
						{ color: waterLight, mX: -3, mY: -2, list: [
							{ sY: 1, fY: true, list: [
								{ sX: { r: 0.4 } },
								{ sX: { r: 0.3 }, fX: true }
							] },
				...
				{ use:"tree-main", color: treeShadow, chance: 0.05, sY:{ a:3, random:10 }, mask: true },
				{ use:"tree-main", color: fruit, chance: 0.06, sY: { r: 0.6, useSize: fruitSY }, sX: { r: 0.6, useSize: fruitSX }, z:10 },
				{ sX: mainTreeSX, sY: mainTreeSY, fX: true, list: [
					{ 
						id:"tree-main", 
						sY: { r: 0.8 }, 
						stripes: { 
		...
		border = function () {
			var edgeDetail = [
				...
				borderVert = [
					{ stripes:{ strip:frameDetailSize }, sX:{r: 0.5}, cY:true, list: borderVertDetail },
					{ stripes:{ strip:frameDetailSize }, sX:{r: 0.5}, rX: true, fX:true, cY:true, list: borderVertDetail },
				],
			...
			return { color:borderColor, z:10000, list:[
				...
				{ sY:borderBottomDetail, mX:borderBottomMargin, list:[
					{},
					{ m:1, color:borderDetailColor }
				] },
				{ sY:borderBottomDetail, mX:borderBottomMargin, fY:true, list:[
					{},
					{ m:1, color:borderDetailColor, list:[
						{ sX:1},
						{ sX:1, fX:true }
