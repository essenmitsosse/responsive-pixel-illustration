export const DrawingTools = function (pixelUnit, getRandom) {
    ...
    this.FillRandom.prototype.draw = function () {
        var width = this.width ? this.width.getReal() : 1,
            ...
            sizeRandom = this.sizeRandom
                ? this.sizeRandom.getReal() + 1
                : false,
            heightRandom = this.heightRandom
                ? this.heightRandom.getReal() + 1
                : false,
            widthRandom = this.widthRandom
            ...
            mask = this.mask
                ? drawingTool.pixelSetter.getMask(this.use)
                : false,
            dontCheck = !mask,
    ...
    this.Panels.prototype.init = (function (pX) {
        return function (args) {
            ...
            while (l--) {
                ...
                newPanels.push({
                    drawer: new drawingTool.Obj().create(
                        { list: current.list },
                        inherit,
                    ),
                    sX: current.sX,
