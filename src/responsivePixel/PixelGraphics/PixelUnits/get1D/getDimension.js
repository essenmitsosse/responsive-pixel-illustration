export const getDimension = (context) => class Dimension {
	dimension = true;
	...
	prepare(args, con) {
		...
		if (objType === 'object') { // is Object
			...
			if (args.getLength) {
				this.realPartCalculation = this.context
					.getGetLengthCalculation(args.getLength[0], args.getLength[1]);
				return;
			}
			...
			if (this.getDefaults(args.r, args.a) && !args.useSize && !args.add) {
				...
				this.realPartCalculation = (args.min || args.max)
					? this.context.getGetRealDistanceWithMaxMinWrapper(args.max, args.min, this.dim)
