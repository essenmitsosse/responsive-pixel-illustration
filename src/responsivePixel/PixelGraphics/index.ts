...
const createVariableList = (variableList) => {
  return {
...
const prepareVariableList = (vl: ReadonlyArray<Size>, pixelUnit: PixelUnit) => {
  if (vl.length === 0) {
    return
  }
  ...
  vl.forEach((current) => {
    if (!current.s) {
      if (!current.autoUpdate) {
        ...
        /* eslint-disable-next-line no-param-reassign */
        current.s = pixelUnit.createSize(current)
      } else {
        /* eslint-disable-next-line no-param-reassign */
...
export class PixelGraphics {
  ...
  constructor(args: {
    ...
    this.pixelUnit.setList(createVariableList(this.variableList))
    if (this.imageFunction.linkList) {
      prepareVariableList(this.imageFunction.linkList, this.pixelUnit)
    }
