export class Axis {
	...
	fromCenter() { return this.pos.getReal() + Math.floor((this.dim - this.realSize) / 2); }
	...
	assignMethods() {
		if (this.center) {
			this.calcPos = this.fromOtherSide
				? this.fromOtherCenter : this.fromCenter;
		} else if (this.toOtherSide) {
			this.calcPos = this.fromOtherSide
				? this.fromOtherToOther : this.toOther;
		} else {
			this.calcPos = this.fromOtherSide
				? this.fromOther : this.normal;
		}
	}
}
