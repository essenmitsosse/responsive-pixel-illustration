...
const zero = { a: 0 },
  ...
  detail = getDarken(shadowColor, 0.4),
  eyes: ColorRgb = [182, 0, 234],
  frame = shadow(eyes),
  red: ColorRgb = [255, 0, 0],
  backgroundColor: ColorRgb = [31, 14, 63],
  zeus: ColorRgb = [240, 240, 240],
  zeusShadow = shadow(zeus),
  flesh: ColorRgb = [190, 160, 160],
  ...
  border = [
    ...
    { minY: 5, list: [{ color: backgroundColor, fY: true, sY: 1, y: 1 }] },
  ],
  clothing = function (down?: boolean) {
    return [
      {
        points: [
          { fX: true, x: { r: 0.2 } },
    ...
    // IMAGE
    {
      ...
      list: [
        // MOTIVE ZEUS
        {
          ...
          color: zeus,
          list: [
            { use: 'background', color: frame, chance: 0.01, s: 1 },
            {
              use: 'background',
              color: frame,
              chance: 0.01,
  ...
  variableList = {
    ...
    bodyRight: { add: ['headSX', mult(-0.5, 'restSY')], min: 'earLength' },
    bodyTop: {
      add: ['headSY', mult(0.4, 'squarySuper'), mult(-1, 'restSX'), 'birdTop'],
      min: { a: 0 },
    },
    ...
    legSX: { r: 1, useSize: 'bodySX', add: [mult(-0.1, 'restSXSuper')] },
    humanLegSY: {
      r: -0.5,
      useSize: 'squarySuper',
      a: 'restSY',
    ...
    // SNAKE BODY
    snakeGap: { add: [mult(0.15, 'restSXMega')], max: mult(0.7, 'bodySY') },
