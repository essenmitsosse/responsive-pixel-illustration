...
var Admin = function ( args ) {
	...
	if ( args.admin && args.socket ) {
		...
		body.setAttribute( "class", ( body.getAttribute( "class" ) || "" ) + " admin" );
	}	
...
Admin.prototype.setupLog = function ( socket ) {
	var infoBarDiv = document.createElement("div"),
		...
		connectionLogList = new this.List( { id: "connectionLog", container: infoBarDiv, tagName: "div" } );
...
Admin.prototype.setupSlides = function ( slides) {
	var currentSlide = ( this.pixel.queryString.slide * 1 ) || 0,
		...
		sideBarContentUl = new this.List( { id: "slides", container: this.sideBarInnerDiv } );
...
Admin.prototype.setupSlider = function () {
	var	slidersDiv = new this.List( { id: "sliders", container: this.sideBarInnerDiv } ),
		...
		slider: function getSliderControl ( slider, span, args ) {
			var value,
				...
				updateInfoSpan = function () {
					...
					span.setAttribute( "style", "left: " + ( ( ( value - min ) / diff ) * 100 - 10 ) + "%;" );
				},
				...
				update = function ( setValue, single ) {
					...
					if( typeof setValue === "number" ) {
						...
						if( single ) {
							...
							parentNode.setAttribute( "style", "color: #000; background-color: #fff;" );
...
Admin.prototype.getButtonCreater = function ( div ) {
	...
	return function createButton ( args ) {
		var button = div.addMessage( 
			...
			( function ( pixel ) { return function () { pixel[ args.functionName ]( args.args ); }; } )( pixel )
		);
...
Admin.prototype.List.prototype.addMessage = function ( message, className, clickEvent ) {
	var newMessage = document.createElement( "li" );

	newMessage.innerHTML = message;
	if( className ) { newMessage.setAttribute( "class", className ); }
