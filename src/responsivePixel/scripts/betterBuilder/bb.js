...
var BB = function ( init ) {
	var args = {},
		...
		random = window.helper.random( init.id || Math.floor( Math.random() * 4294967296 ) );
...
BBProto.Overview = function ( init ) {
	...
	this.ll.push(
		...
		this.innerS = { r:inner, useSize: this.outerSX, max:{ r:inner, useSize: this.outerSY }, odd:true }
