...
Builder.prototype.Person = function (args) {
	const that = this;
	const nr = this.nr = this.basic.objectCount += 1;
	...
	this.vL[`topSY${nr}`] = this.mult((this.bigHead ? 0.7 : 0.2) * this.superHead, `personSqu${nr}`);
	this.vL[`bottomSY${nr}`] = [`personSqu${nr}`, this.sub(`topSY${nr}`)];

	this.vL[`personSX${nr}`] = this.mult(this.FL() * 0.2 + 0.1, `personHalfSX${nr}`);
	this.vL[`personSideSX${nr}`] = this.mult(1.2, `personSX${nr}`);
	this.vL[`personSY${nr}`] = this.mult(this.dwarf ? 0.7 : 1, `personSqu${nr}`);

	this.vL[`headSX${nr}`] = { r: this.IF() ? 0.7 : 0.3, useSize: `topSY${nr}`, min: 2 };
	this.vL[`headNeckSY${nr}`] = { r: 1, useSize: `topSY${nr}`, min: 3 };

	this.vL[`headSideSX${nr}`] = this.mult(2, `headSX${nr}`);

	this.vL[`neckSX${nr}`] = { r: 0.4, useSize: `headSX${nr}`, max: [`personSX${nr}`, -1] };
	this.vL[`neckSY${nr}`] = this.mult(this.IF() ? 0.2 : 0.1, `headNeckSY${nr}`);

	this.vL[`headSY${nr}`] = [`headNeckSY${nr}`, this.sub(`neckSY${nr}`)];

	this.vL[`faceSX${nr}`] = {
		r: this.IF(0.4) ? 0.3 : 0.6, useSize: `headSX${nr}`, a: 1, max: `headSX${nr}`, min: 2,
	};
	...
	this.vL[`eyeX${nr}`] = this.IF(0.5) ? { r: 0.5, useSize: `faceSX${nr}`, max: [`faceSX${nr}`, -2] } : { a: 0 };
	this.vL[`eyeS${nr}`] = { r: 0.5, useSize: `faceSX${nr}`, max: { a: 3, max: [`faceSX${nr}`, this.sub(`eyeX${nr}`), -1] } };

	this.vL[`faceSideSX${nr}`] = { r: 0.35, useSize: `headSideSX${nr}`, min: `eyeS${nr}` };
	...
	this.vL[`torsoSY${nr}`] = { r: this.IF() ? 0.2 : 0.3, useSize: `bottomSY${nr}`, max: [`bottomSY${nr}`, this.sub(`legSY${nr}`)] };

	this.vL[`bodySY${nr}`] = [`legSY${nr}`, `torsoSY${nr}`];

	this.vL[`legWid${nr}`] = { r: 0.3, useSize: `personSX${nr}`, min: 1 };
	this.vL[`crotchSY${nr}`] = this.IF(0.3) ? {
		r: 1.5, useSize: `personSX${nr}`, min: 1, max: this.mult(0.5, `legSY${nr}`),
	} : `legWid${nr}`;
	...
	this.vL[`shoulderSideSX${nr}`] = {
		r: 0.4, useSize: `personSX${nr}`, min: `armWid${nr}`, max: [`armWid${nr}`, 1],
	};
...
Builder.prototype.Person.prototype.draw = function (args) {
	const nr = args.nr = this.nr;
	const back = args.back = args.view === 'back';
	const side = args.side = args.view && !back;
...
Builder.prototype.BasicBody = function (args) {
	...
	args.clothColor = new this.Color(this.IF(0.3) ? this.IF(0.3) ? 0 : 1 : 2, this.IF() ? this.IF() ? 4 : 2 : 3);
...
Builder.prototype.Head = function (args) {
	...
	this.hairColor = args.hairColor = (this.IF(0.5) ? args.clothColor : args.skinColor).copy({ brContrast: this.IF() ? 2 : -2 });
	this.shortBeardColor = this.skinColor.copy({ brContrast: -1 });

	// Assests
	this.face = new this.basic.Face(args);
	this.headGear = !args.animal ? this.IF() ? new this.basic.BasicHat(args) : this.IF(0.05) ? new this.basic.Helmet(args) : undefined : undefined;
		...
		list: [
			...
			// Head
			{
				...
				list: [
					...
					{
						color: this.hairColor.get(), fX: true, sX: !args.back && (side ? `hairSideSX${nr}` : `hairSX${nr}`), sY: this.beard ? undefined : this.mult(0.5, `headSY${nr}`),
					},
...
Builder.prototype.Face = function (args) {
	...
	// Assests
	this.beard = this.IF();
	this.moustach = ((this.beard && this.IF(0.6)) || this.IF()) && new this.basic.Beard(args);

	if (this.beard) { this.vL[`beardOvershotSideSX${nr}`] = this.mult(0.2, `hairSideSX${nr}`); }
}; // END Face
...
Builder.prototype.Face.prototype.draw = function (args) {
	...
	const { eyes } = args;
	const { teeth } = args;
	const eyesClosed = eyes === 'closed' || (args.right && eyes === 'wink');
	const eyesAngry = eyes === 'angry';
	const eyesHappy = eyes === 'happy';
	const eyesSquish = eyesAngry || eyesHappy;
	const mouthOpen = mouth === 'open';
	const mouthGrin = mouth === 'grin';
	const mouthD = mouth === 'D:';
	const mouthNarrow = args.mouthWid === 'narrow' || (!args.right && eyes === 'wink');
	const mouthHappy = mouth === 'happy';
	const mouthSad = mouth === 'sad';
	const mouthClosed = mouthHappy || mouthSad;
	const teethUp = !mouthClosed && teeth === 'up' || teeth === 'both';
	const teethDown = !mouthClosed && teeth === 'down' || teeth === 'both';
	const teethFull = !mouthClosed && teeth === 'full';
	...
	return {
 	...
 	z: args.zInd + (args.back ? -60 : 60),
						sX: (mouthNarrow && { r: 0.4, a: 1, max: { r: 1 } }) || (mouthClosed && { r: 1, a: -1 }),
						y: mouthHappy && 1,
						list: [
							(mouthGrin || mouthD) && {
								name: 'Dot', fY: mouthGrin, fX: true, clear: true,
							},
							{ color: teethFull && this.teethC.get() },
							teethUp && { color: this.teethC.get(), sY: { r: 0.3, max: 1 }, sX: { r: 0.8 } },
							teethDown && {
								color: this.teethC.get(), sY: { r: 0.2, max: 1 }, sX: { r: 0.7 }, fY: true,
							},
							mouthClosed && {
								name: 'Dot', y: mouthHappy ? -1 : 1, x: -1, fX: true,
							},
						],
					},
 	...
 	color: this.hairColor.get(),
				  ...
				  y: mouthClosed && -1,
				  sX: side ? [`headSideSX${nr}`, `beardOvershotSideSX${nr}`, this.sub(`hairSideSX${nr}`)] : `headSX${nr}`,
 	...
 	sX: side ? `beardOvershotSideSX${nr}` : `hairSX${nr}`,
				    ...
				    { 	fX: true, stripes: { random: { r: 0.5 }, change: { r: 1 }, seed: nr * 2 } },
				  ],
				},
				],
			},
...
Builder.prototype.Ear = function (args) {
	...
	// Precalc Sizes
	this.vL[`earSX${nr}`] = { r: this.FL() * (this.ear ? 0.2 : 2), useSize: `faceSX${nr}`, min: 1 };
	this.vL[`earSY${nr}`] = { r: this.FL() * 0.4, useSize: `faceSY${nr}`, min: 1 };
}; // END Ear
...
Builder.prototype.Ear.prototype.draw = function (args) {
	...
	&& [
	  {},
	  {
	    sY: { r: 0.3, useSize: `earSX${nr}` }, sX: { r: 1, useSize: `earSY${nr}` }, fX: true, tY: true,
	  },
	],
	};
}; // END Ear draw
...
Builder.prototype.BasicHat.prototype.draw = function (args) {
	...
	return {
		...
		list: [
			...
			!this.hatBasecap && this.hatRim && {
				fY: true, y: 1, sY: { r: 0.2 }, color: this.hatRimColor.get(),
			},
			...
			this.hatBasecap
				...
				:			this.hatRim
				&& { fY: true, sY: { r: 0.1, min: 1 }, mX: this.mult(this.hatNarrow ? -0.5 : -1, `headSX${nr}`) },
		],
	};
}; // END BasicHat draw
...
Builder.prototype.Helmet = function (args) {
	const brightArmor = this.IF(0.05);

	this.nr = args && args.nr;
	this.mainC = args.clothColor.copy({ brContrast: brightArmor ? 2 : this.IF(0.1) ? 2 : -2 });
}; // END Helmet
...
Builder.prototype.Helmet.prototype.draw = function (args) {
	...
	[
	  ...
	  { sX: side ? [`faceSideSX${nr}`, -1] : [`faceSX${nr}`, this.sub(`eyeS${nr}`), this.sub(`eyeX${nr}`)], sY: { r: 0.6 } },
	  {
	    sY: { r: 1.1 },
	    list: [
...
Builder.prototype.UpperBody = function (args) {
	...
	this.satchelColor = args.beltColor = this.satchel ? this.bodyColor.copy({ brContrast: this.IF(0.1) ? 2 : -2, nextColor: this.IF() }) : undefined;

	// Assests
	this.armLeft = new this.basic.Arm(args);
	this.armRight = new this.basic.Arm(args, true);

	this.chestDeko = args.topless ? new this.basic.Nipple(args) : this.IF(0.1) ? new this.basic.Buttons(args) : undefined;
...
Builder.prototype.Nipple.prototype.draw = function (args) {
	...
	return {
		color: this.skinDetailColor.get(), name: 'Dot', x: { r: side ? 0.2 : 0.5 }, y: this.mult(0.4, `torsoSY${nr}`),
	};
}; // END Nipple draw
...
Builder.prototype.Arm = function (args, right) {
	...
	// Assets
	this.tool = this.IF(0.05) && new this.basic.SquareObject(args, this.name) || this.IF(0.05) && new this.basic.Sword(args, this.name);
}; // END Arm
...
Builder.prototype.Arm.prototype.draw = function (args) {
	...
	const { side } = args;
	const up = false;
	const shoulder = args.shoulder && args.shoulder[this.name];
	const shoulder90 = shoulder === -90;
	const ellbow = args.ellbow && args.ellbow[this.name];
...
Builder.prototype.Sword = function (args, name) {
	const nr = this.nr = args && args.nr;
	const nrName = this.nrName = nr + name;
...
Builder.prototype.Sword.prototype.draw = function (args) {
	...
	return {
		...
		list: [
			...
			this.noKnife && {
				x: [`handleSX${nrName}`, -2], sX: 1, sY: 5, cY: this.noKnife,
			},
		],
	};
}; // END Sword draw
...
Builder.prototype.SquareObject = function (args, name) {
	...
	// Colors
	this.shieldColor = args.skinColor.copy({ nextColor: this.IF(0.5), brContrast: this.IF() ? 2 : -2 });
	this.shieldColorDetail = this.detail && this.shieldColor.copy({ prevColor: this.IF(0.8), brContrast: this.IF() ? 2 : -2 });

	// Precalc Sizes
	this.vL[`shieldSX${nrName}`] = { r: this.FL() * 0.6, useSize: `armSY${nr}`, min: `armWid${nr}` };
	this.vL[`shieldSY${nrName}`] = { r: this.FL() * 0.6, useSize: `armSY${nr}`, min: 1 };
	this.handle && (this.vL[`handleSY${nrName}`] = { r: this.FL() * 0.7, useSize: `armSY${nr}` });
}; // END SquareObject
...
Builder.prototype.SquareObject.prototype.draw = function (args) {
	...
	return {
		...
		list: [
			...
			this.handle && {
				sX: this.handleWide ? `shieldSX${nrName}` : this.handleSmall ? 1 : [2, `armWid${nr}`],
				cX: true,
				list: [
					{ sY: 1 },
					{
...

			{
				...
				tY: true,
				fY: true,
				y: (this.atMiddle && this.mult(0.5, `shieldSY${nrName}`, 1)) || (this.handle && { r: -1, useSize: `handleSY${nrName}`, a: 1 }),
				z: args.zInd + 5,
				...
				list: [
					...
					{},
					this.detail && {
						mX: { r: this.detailMX }, sY: { r: this.detailSY }, mY: { r: this.detailMY }, color: this.shieldColorDetail.get(),
					},
				],
			},
		],
...
Builder.prototype.LowerBody = function (args) {
	...
	// Colors
	this.skinColor = args.skinColor;
	this.pantsColor = args.pantsColor = args.pantsColor || (this.noPants ? false : args.clothColor.copy({ nextColor: this.IF(0.6), brContrast: this.IF() ? 1 : -1 }));
	...
	return {
		...
		list: [
			...
			this.belt && {
				...
				sY: {
					r: 0.3, useSize: `crotchSY${nr}`, min: 1, max: 2,
				},
				color: this.beltColor.get(),
			},

			this.highPants && { sY: { r: 0.5, useSize: `torsoSY${this.nr}` }, tY: true },
...
Builder.prototype.Leg.prototype.draw = function (args) {
	...
	return {
		...
		list: [
			...
			this.shortPants ? { color: skinColor.get(), fY: true, sY: { r: 0.5 } } : undefined,

			// FEET
			{ color: (this.shoeColor || skinColor).get(), fY: true, sY: this.boots ? { r: 0.5 } : { r: 1, otherDim: true } },
		],
	};
}; // END Leg draw
