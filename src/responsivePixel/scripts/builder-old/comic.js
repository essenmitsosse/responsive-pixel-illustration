...
Builder.prototype.Comic = function ( init ) {
	var list = [],
		args = {},
		panels = this.IF(.05) ? 1 : this.GR(3,6),
		...
		outerBorderColor = this.IF(.9) && borderColor.copy({ brContrast:this.IF(.8) ? 5 : 3, dontChange:this.IF(.5) }),
		bigPanel = panels % 2 !== 0 && ( this.IF(.5) ? 0 : panels - 1 ),
	...
	this.vL["_panelOvershot2"] = { add:["imgSY",{ r:this.R(-2.5,-1.5), useSize:"_panelWid" }], min:{a:0}, max:1 };
	this.vL["_panelOvershot2IF"] = { r:10000000, useSize:"_panelOvershot2" };

	this.vL["_panelOvershot3"] = { add:["imgSY",{ r:-8, useSize:"_panelWid" }], min:{a:0}, max:1 };
	this.vL["_panelOvershot3IF"] = panels === 4 ? { a:0 } : { r:10000000, useSize:"_panelOvershot3", min:{a:0} };

	this.vL["gutterX"] = { r:gutter, useSize:"imgSqu", min:3, max:["borderX",-1] };
	this.vL["gutterY"] = { r:gutter * horRatio, useSize:"imgSqu", min:["gutterX",1], max:["borderY",-1] };

	this.vL["gutterBorderS"] = { r:this.GR(-1,1), useSize:"gutterX", max:-1, min:{r:-.5, useSize:"gutterX", a:.5 } },

	this.vL["_effectiveImgSX"] = { max:{r:3, useSize:"imgSX"}, add:["_panelOvershot3IF",{ a:"_panelOvershot2IF", min:"imgSX", max:{r:2, useSize:"imgSX"} }] };
	...
	this.vL["panelSY"] = { 
		add:[
			{ add:[ "imgSY", this.mult(-1,"_panelOvershot2IF") ], min:this.mult(.5,"panelRest2SY")  },
	...
	this.vL["2rowX"] = { r:-1, useSize:"_panelOvershot2IF", add:["_panelOvershot3IF"],min:[ { r:-panelsCalc/2, useSize:"panelSX" }, { r:-panelsCalc/2, useSize:"gutterX" } ], max:{a:0} };
	this.vL["2rowY"] = { a:"_panelOvershot2IF", add:[this.sub("_panelOvershot3IF")], max:["panelSY","gutterY"], min:{a:0} };

	this.vL["3rowX"] = { r:-1, useSize:"_panelOvershot3IF", min:[ { r:-panelsCalc/3, useSize:"panelSX" }, { r:-panelsCalc/3, useSize:"gutterX" } ] };
	this.vL["3rowY"] = { a:"_panelOvershot3IF", max:["panelSY","gutterY"] };

	this.vL["fullSX"] = [
		{ r:panelsCalc, useSize:"panelSX" }, 
		{ r:panelsCalc-1, useSize:"gutterX" },
		{ r:-1, useSize:"_panelOvershot2IF", add:["_panelOvershot3IF"], min:[{ r:-panelsCalc/2, useSize:"panelSX" },{ r:(-panelsCalc)/2, useSize:"gutterX" },] },
		{ r:-1, useSize:"_panelOvershot3IF", min:[{ r:-2*panelsCalc/3, useSize:"panelSX" },{ r:(-2*panelsCalc)/3, useSize:"gutterX" },] },
	];
...
Builder.prototype.Panel = function ( args ) {
	// Forms & Sizes
	this.horizont = this.IF(.6) ? this.R(.1,.6) : this.R(0,1);

	this.lightnessSwitched = this.IF( .01 * args.panels );
	this.colorChangeGradual = !this.lightnessSwitched && this.IF( .01 * args.panels );

	if( this.lightnessSwitched ) {
		this.lightnessSwitchPanel = this.R(1,args.panels-1);
		this.backWard = this.IF(.5);
		this.newLightness = args.newLightness = args.newLightness || this.GR(1,3) * ( this.IF(.7) ? -1 : 1 );
	} else if( this.colorChangeGradual ) {
		this.colorChangeStart = (
			this.IF(.5) ? 
				0 
				: ( this.IF(.8) ? -1 : 1 ) * this.R(0,4)
			);

		this.colorChange = this.R(0,.5) * ( 
			this.colorChangeStart === 0 ? 
				this.IF( .8 ) ? -1 : 1
				: this.colorChangeStart > 0 ? -1 : 1 
		);
	}

	this.clearSky = this.IF(.5);

	this.sun = this.clearSky && this.IF(.5);

	// Colors
	this.skyColor = args.skyColor = args.borderColor.copy({nextColor:this.IF(.1), brSet:this.sun ? 4 : 5 });
	if( this.sun ) { 
	...
	this.actor1 = this.IF(.99) && new this.basic.Actor( args );
	this.actor2 = this.IF(.9) && new this.basic.Actor( args );
	this.actor3 = this.IF(.1) && new this.basic.Actor( args );

	this.forrest = this.IF(.4) && new this.basic.Forrest( args );
...
Builder.prototype.Panel.prototype.draw = function ( args ) {
	var nr = args.i,
		closeUp = this.IF(.1),
		wideShot = !closeUp && this.IF(.2),
		superWideShot = !closeUp && !wideShot && this.IF(.1),
		actionBackground = this.IF( .01 + ( closeUp ? .3 : 0 ) + ( wideShot ? .05 : 0 ) ) && 
	...
	if( this.colorChangeGradual || this.lightnessSwitched ) {
		...
		if( this.lightnessSwitched ) {
			darkness = this.lightnessSwitchPanel >= nr ? 
				!this.backWard ? this.newLightness : 0
				: this.backWard ? this.newLightness : 0;
		} else {
		...
		if( darkness === -1 ) {
			// Dusk
			this.finalSkyColor = this.skyColor.copy({ brSet:3, dontChange:true, nextColor:true });
		} else if ( darkness < -1 ) {
			// Night
			...
			if( stars ) {
				...
				this.starColor = this.finalSkyColor.copy({ brSet:3, nextColor:true, dontChange:true });
			}
		}
	} else {
		darkness = 0;
	...
	this.vL["thingS"+nr] = this.mult(closeUp ? 2 : wideShot ? .5 : superWideShot ? .2 : 1,"panelSqu");
	this.vL["horizont"+nr] = {
		r:this.horizont * ( closeUp ? .6 : wideShot ? 1.25 : superWideShot ? 1.5 : 1 ), 
		useSize:"panelSY" 
	};
	...
	actors = {
		...
		x:{r:this.IF(.5) ? .2 : -.2, useSize:"thingS"+nr},
		y:wideShot || superWideShot ? 
			{ r:.5, useSize:"horizont"+nr }
			: {r:closeUp ? -.5 : -.1, useSize:"thingS"+nr},
		list:[
			this.actor1 && {
				s:"thingS"+nr,
				cY:true,
		...
		list:[
			...
			{ 
				...
				list:[
					...
					// Sun
					this.sun && ( wideShot || superWideShot ) && {
						s:{r:.1, useSize:"panelSY"},
						x:{r:.2},
						y:{r:.2},
...
Builder.prototype.Actor.prototype.draw = function ( args, z, size ) {
	...
	args.shoulder = { 
		left: this.IF(.7) && Math.pow( this.R( 0, 1 ), 3 ), 
		right: this.IF(.7) && Math.pow( this.R( 0, 1 ), 3 )
	};

	args.arm = { 
		left: args.shoulder.left > .55  ? this.R( .5 ) : this.IF(.7) ? this.R(-.75,.75) : this.R(-.25,.25), 
		right: args.shoulder.right > .55  ? this.R( .5 ) : this.IF(.7) ? this.R(-.75,.75) : this.R(-.25,.25) 
	};
	...
	args.leg = {};
	args.leg[ this.IF(.5) ? "right" : "left" ] = legPos[ this.GR( 0, legPos.length ) ];

	args.hatDown = this.IF(.02);
...
Builder.prototype.Ground = function ( args ) {
	...
	// Colors
	this.groundColor = args.groundColor = args.skyColor.copy({ nextColor:this.IF(), brContrast:this.GR(1,4) });
