	...
	var help = helper,
		...
		getRow = function( cols ) {
			var all = [];
				...
				while( cols -- ) {
					all.push( { list:getCols( builder.Person, true, cols ), x:mult( fieldsCount * cols, "personS" ) } );
				}
		...
		getCols = function ( func, isCol, nr, nr2 ) {
			...
			while( i < max ) {
				...
				if( !isCol ) {
					...
					args.shoulder = { 
						left: IF(.8) && Math.pow( R(0,1), 3 ), 
						right: IF(.8) && Math.pow( R(0,1), 3 )
					};

					args.arm = { 
						left: args.shoulder.left > .55  ? R(0,.5) : IF(.8) ? R(0,1) * 1.5 - .75 : R(0,1) * .5 - .25, 
						right: args.shoulder.right > .55  ? R(0,.5) : IF(.8) ? R(0,1) * 1.5 - .75 : R(0,1) * .5 - .25 
					};
					...
					args.leg = {};
					args.leg[ IF(.5) ? "right" : "left" ] = legPos[ GR( 0, legPos.length ) ]
