...
Builder.prototype.Arm = function (args) {
	...
	this.toolLeft = (args.demo || this.IF(0.1)) && new (this.IF(0.5) ? this.basic.Shield : this.basic.Sword)(args);
	this.toolRight = (!args.demo && this.IF(0.1)) && new (this.IF(0.5) ? this.basic.Shield : this.basic.Sword)(args, true);
...
Builder.prototype.Arm.prototype.draw = function (args, rightSide, behind) {
	...
	if (args.calc) {
		...
		this.vL[`shoulderSY${nr}`] = {
			r: 1, useSize: `armSX${nr}`, min: 1, max: `chestSY${nr}`,
		};
		this.vL[`shoulderFullSX${nr}`] = [this.mult(sideView ? 2 : 1, `shoulderSX${nr}`), `chestSX${nr}`];

		this.vL[`handSX${nr}`] = { add: [`armSX${nr}`, 1], min: 1, max: { r: 0.1, useSize: `personHalfSX${nr}` } };
	...
	this.vL[`armHalfSX${nrName}`] = { r: renderFromRight ? 0.49 : 0.51, useSize: `armSX${nr}`, max: { r: 0.22, useSize: `upperBodySX${nr}`, a: renderFromRight ? -1 : 0 } };
						...
						list: [
							...
							// Hand
							{
								...
								list: [
									...
									// Finger
									!tool && finger && { sX: 1, sY: { r: 1.5, a: 1, max: { r: 0.15, useSize: `personHalfSX${nr}` } }, fX: true },
...
Builder.prototype.ShoulderPad.prototype.draw = function (args, z) {
	...
	return {
		sX: {
			r: this.SX, useSize: `personHalfSX${nr}`, min: `armSX${nr}`, save: `shoulderPadSX${nr}`,
		},
		sY: { r: this.SY, useSize: `armSX${nr}`, min: { r: 0.2, useSize: `shoulderPadSX${nr}` } },
		...
		list: [
			...
			this.topDetail && {
				...
				sX: this.topDetailStrip ? { r: 1, a: -2 } : { r: 0.2, min: 1, save: `shoulderPadDetailSX${nr}` },
