...
Builder.prototype.LowerBody.prototype.draw = function (args, z) {
	...
	if (args.calc) {
		this.vL[`crotchSY${nr}`] = {
			r: this.crotchSY, useSize: `legSX${nr}`, max: this.mult(0.4, `lowerBodySY${nr}`), min: 1,
		};
	}
...
Builder.prototype.Belt.prototype.draw = function (args, z) {
	...
	return {
		z: z + 115,
		sY: {
			r: this.beltSY, useSize: `crotchSY${nr}`, a: 1, max: { r: 1.4, useSize: `legSX${nr}`, max: [`crotchSY${nr}`, -1] },
		},
...
Builder.prototype.Skirt.prototype.draw = function (args, z) {
	...
	if (args.calc) {
		this.vL[`skirtSY${nr}`] = {
			r: this.skirtSY, useSize: `lowerBodySY${nr}`, min: 1, max: `lowerBodySY${nr}`,
		};
...
Builder.prototype.Leg.prototype.draw = function (args, z, rightSide, behind) {
	...
	if (args.calc) {
		this.vL[`legSX${nr}`] = {
			r: this.legSX, useSize: `personHalfSX${nr}`, max: [`lowerBodySX${nr}`, this.legsIn ? -2 : -1], min: 1,
		};
		this.vL[`upperLegSY${nr}`] = { r: 0.5, useSize: `lowerBodySY${nr}` };
		this.vL[`lowerLegSY${nr}`] = [`lowerBodySY${nr}`, this.sub(`upperLegSY${nr}`)];

		this.vL[`shoeSY${nr}`] = this.bootsSY ? { r: this.bootsSY, useSize: `lowerLegSY${nr}`, min: 1 } : { r: 0.8, useSize: `legSX${nr}`, min: 1 };

		this.vL[`legMaxSX${nr}`] = {
			r: sideView ? 0.8 : 1, useSize: `lowerBodySX${nr}`, a: -2, max: [`legSX${nr}`, 2], min: `legSX${nr}`,
		};

		this.vL[`thighsSX${nr}`] = {
			useSize: `personHalfSX${nr}`, r: this.thights || 1, max: `legMaxSX${nr}`, min: 1,
		};
		this.vL[`calvesSX${nr}`] = {
			useSize: `personHalfSX${nr}`, r: this.calves || 1, max: `legMaxSX${nr}`, min: 1,
		};
								...
								list: [
									{
										...
										list: [
											...
											{
												sY: `shoeSY${nr}`,
												sX: !this.bareFoot && this.bootsSXBig && { r: 1, a: this.bootsSXBig, max: { r: 0.5, useSize: `personHalfSX${nr}` } },
												cX: true,
												fX: true,
												tX: true,
												fY: true,
												...
												list: [
													...
													!this.bareFoot && {
														sY: { r: 0.2, useSize: `legSX${nr}`, min: 1 },
														sX: { r: 1, a: 1, max: { r: 0.15, useSize: `personHalfSX${nr}`, max: [`calvesSX${nr}`, 1] } },
														fY: true,
													},
												],
											},
