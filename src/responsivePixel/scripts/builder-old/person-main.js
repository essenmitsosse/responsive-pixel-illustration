...
Builder.prototype.Person.prototype.draw = function ( args, z ) {
	...
	this.vL[ "personHalfSX"+nr ] = { r:.5, min:5, useSize:args.size };
...
Builder.prototype.BasicBody = function ( args ) {
	var nextFirstColor = this.IF(.5),
		nextSecondColor = this.IF(.2),
	...
	this.lowerBodySY = ( this.IF(.1) ? this.R( .5, .9 ) : .7 );
	if( args.demo && args.hip ) {
		this.lowerBodySY = args.hip;
	}

	this.animal = args.animal = this.IF(.02);
	...
	this.firstColor = args.firstColor = args.skinColor.copy( { 
		nr:hues[1], 
		brContrast:( this.IF(.5) ? -1 : 1 ) * ( this.IF(.8) ? 1 : 2 ), 
		min:this.IF() ? 0 : 1, 
		max:4 
	} );
	...
	this.secondColor = args.secondColor = args.skinColor.copy( { 
		nr:hues[2], 
		brContrast:( this.IF(.8) ? -1 : 1 ) * ( hues[1] === hues[2] ? 1 : 2 ),
		max:4 
	} );
...
Builder.prototype.BasicBody.prototype.draw = function ( args, right ) {
	...
	if( args.calc ) {
		this.vL[ "bodyRestSY"+nr ] = { a:"personSY"+nr, max:[ args.size, this.sub( "headMaxSY"+nr ), this.sub( "neckSY"+nr ) ] }

		this.vL[ "lowerBodySY"+nr ] = { r:this.lowerBodySY, useSize:"bodyRestSY"+nr, min:1 };
		this.vL[ "upperBodySY"+nr ] = { add:[ "bodyRestSY"+nr, this.sub( "lowerBodySY"+nr ) ], min:1 };
...
Builder.prototype.Logo = function ( args, right, symetrical, logoColor ) {
	...
	this.oneSide = !symetrical && this.IF(.1);
	this.oneSide && ( this.side = this.IF(.5) );

	this.roundUp = this.IF(.3);
	this.roundDown = this.IF(.3);
	this.dentUp = this.IF(.3);
	this.dentDown = this.IF(.3);
	this.stripUp = this.IF(.1);
	this.stripDown = this.IF(.1);
	this.stripSide = this.IF(.1);
	this.edgeUp = this.IF(.2);
	this.edgeDown = this.IF(.2);

	// Color
	this.logoColor = logoColor || args.clothColor.copy({
		nextColor:color,
		brSet:args.clothColor.getBr() + ( ( this.IF(.5) ? -1 : 1 ) * ( !color || this.IF(.2) ? 2 : 1 ) )
	})
	// Assets
...
Builder.prototype.Logo.prototype.draw = function ( args ) {
	...
	return ( !this.oneSide || args.right === this.side ) && {
		...
		list:[
			...
			this.roundDown && { fX:true, name:"Dot", fY:true, clear:true },
			...
			this.edgeUp && { sX:{r:.4}, sY:{r:.4}, clear:true },
			this.edgeDown && { sX:{r:.4}, sY:{r:.4}, fY:true, fX:true, clear:true },
			{},
		]
	};
