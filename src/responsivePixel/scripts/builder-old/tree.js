...
Builder.prototype.TreeFamily = function (args) {
	...
	this.trunkColor = args.groundColor
		? (args.skyColor && this.IF(0.2) ? args.skyColor : args.groundColor).copy({ brSet: Math.floor(this.GR(1, 4)) })
	...
	this.foliageColor = args.skyColor
		? (args.groundColor && this.IF(0.8) ? args.skyColor : args.groundColor).copy({ brContrast: this.IF() ? -2 : 2 })
	...
	this.fruitColor = this.fruit && this.foliageColor.copy({ nextColor: this.IF(0.8), brContrast: 2 });
};
...
Builder.prototype.Tree.prototype.draw = function (args, z, size) {
	...
	return [
		...
		// Trunk
		{
			cX: true,
			sX: {
				r: this.trunkSX, useSize: `treeSqu${code}`, min: 1, save: `trunkSX${code}`,
			},
						...
						list: [
							{
								sY: `trunkSX${code}`,
								fY: true,
								sX: { r: this.crookedSX, useSize: `trunkSX${code}` },
								list: [
									{ },
									{
...
Builder.prototype.Tree.prototype.addBranches = function (hor, parentLeft, count, level) {
	...
	const strip = {
		stripes: {
			random: { r: -0.5 }, strip: this.leaveX, change: { r: this.leaveChange }, seed: this.nr + count + level, cut: true,
		},
	};
	const { code } = this;
	const leaves = [
			...
			list: [
				{
					fY: true,
					stripes: {
						strip: this.leaveX, random: { r: -1 }, change: { r: this.leaveChange * 5 }, cut: true,
					},
				},
			],
		},
		...
		list: [
			...
			(level < 3) && {
				...
				list: [
					...
					{
						sX: { r: 0.5, a: this.leaveX }, fX: true, rX: true, list: leaves,
					},
				],
			},
		],
	...
	if (i > 0 && level <= 3) {
		while (i--) {
			list.push({
				sX: { r: this.getRandom() * this.shorter * (!hor ? this.horFactor : 1) * (level === 1 ? this.minFoliagePos + 0.2 : 1), otherDim: true, min: 1 },
