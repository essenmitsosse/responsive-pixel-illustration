...
Builder.prototype.Head.prototype.getSizes = function ( args ) {
	...
	if( args.calc ) {
		...
		args.headMinSY = this.pushLinkList( { r: this.headSY, useSize: args.headBaseSY, a: 1, min: 1 } );
		args.headMinSX = this.pushLinkList( { r: this.headSX, min: 1, useSize: args.headMinSY, a: 1.4 } );
		...
		args.neckSY = this.pushLinkList( { r: this.headSY * this.neckSY, useSize: args.size, a: -1, min: { a: 0 } } );
...
Builder.prototype.Eye.prototype.getSizes = function ( args ) {
	...
	if( args.calc ) {
		...
		args.eyeSY = this.pushLinkList( { r: this.eyeSX * this.eyeSY, useSize: args.headMinSX, min: { r: 0.2, useSize: args.headMinSY, max: 1 }, max: { r: 2, useSize: args.eyeSX, a: -1} } );
...
Builder.prototype.Mouth.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		args.mouthSX = this.pushLinkList( { r: this.mouthSX * ( sideView ? 0.7: 1 ), a: 0.5, useSize: args.headMinSX, max: args.headMinSX } );
	...
	return !args.backView && { 
		...
		list: mouthSmile ?
			...
			teethFull && { sX: { r: 0.75, min: { r: 1, a: -2, min: 2 } }, color: this.teethColor.get(), list: [
				{},
					...
					this.hairSide && ( 
						{
							...
							sY: { r: this.hairSY, useSize: args.headMinSY, min: args.hairSideSY, max: [ args.personRealMinSY,-2] },
							list: [
								{ 
...
Builder.prototype.Hat.prototype.draw = function ( args ) {
	...
	return {
		...
		cX: sideView,
		fX: sideView,
		sY: { r: this.hatSY, useSize: args.headMinSY, min: [{ r: this.hatDepthY, useSize: args.foreheadSY, min: 1, save: "hatDepthY"+nr },1] },
		...
		list: [
			{
				list: [
					...
					// Top Detail
					this.topDetail && { tY: true, sX: { r: 0.2, min: 1}, sY: 1, cX: sideView, color: this.helmDetailColor.get() },

					// Top Part
					{ sY: { r: 1, max: args.foreheadSY } },
					
					// Sides
					this.sides && { sX: { a: args.eyeOutX, min: 1}, fX: true, list: this.bottomDetail && [
						{},
					...
					this.nosePiece && { 
						z: 5, 
						sX: { r: 0.2, useSize: args.headSX, max: args.eyeX, min: [ args.eyeX,-1] }, 
			...
			this.foreheadDetail && { 
				sY: { r: this.foreheadDetailSY, useSize: args.foreheadSY, min: 1, save: "helmDetailSX"+nr }, 
				y: { r: 0.7, a: -1, useSize: args.foreheadSY, min: { a: 0}, max: { r: -1.2, useSize: args.helmDetailSX, a: args.foreheadSY } }, 
...
Builder.prototype.HeadBand.prototype.draw = function ( args, z ) {
	...
	return {
		z: z,
		sY: { r: 0.3, useSize: args.foreheadSY, min: 1, save: "headBandSX"+nr },
		sX: args.hairSX,
		cX: sideView,
