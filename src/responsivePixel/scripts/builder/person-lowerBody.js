...
Builder.prototype.LowerBody = function ( args ) {
	...
	this.skirt = args.skirt = ( args.demo || this.wideHips || this.IF( args.animal ? 0.05: 0.15 ) ) && new this.basic.Skirt( args );
	if ( !args.animal && this.IF( 0.3) ) { this.belt = new this.basic.Belt( args ); }
...
Builder.prototype.LowerBody.prototype.draw = function ( args, z ) {
	...
	leg = { sY: this.skirt && args.feetRestSY, list: [ // Check if Skirt is so long, that it covers all the feet
		this.leg.draw( args, z, args.right ),
	]};

	if( args.calc ) {
		args.crotchSY = this.pushLinkList( { r: this.crotchSY, useSize: args.legSX, max: this.mult( 0.4, args.lowerBodySY ), min:1 } );
	}
...
Builder.prototype.Belt = function ( args ) {
	...
	if( this.buckle ) { this.buckleColor = this.beltColor.copy({ brContrast: this.IF( 0.5) ? -1: 2 }); }
	if( this.strips ) { this.pantsColor = args.skirt ? args.skirtColor: args.pantsColor; }
...
Builder.prototype.Belt.prototype.draw = function ( args, z ) {
	...
	return {
		z: z + 115,
		sY: { r: this.beltSY, useSize: args.crotchSY, a:1, max: { r:1.4, useSize: args.legSX, max: [ args.crotchSY,-1]} },
		...
		list: [
			...
			!args.backView && this.buckle && {
				...
				sX: { r: this.buckleSX * ( sideView ? 0.5: 1 ), min: { r:1, otherDim: true, max: { r: 0.8, a:-1, otherDim: true} }},
			}
		]
	};
...
Builder.prototype.Skirt = function ( args ) {
	...
	if( this.stripes ) { this.stripeColor = this.skirtColor.copy({ brContrast:-1}); }
...
Builder.prototype.Skirt.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		args.skirtSY = this.pushLinkList( { r: this.skirtSY, useSize: args.lowerBodySY, min:1, max: args.lowerBodySY } );
...
Builder.prototype.Leg.prototype.draw = function ( args, z, rightSide, behind ) {
	...
	if( args.calc ) {
		args.legSX = this.pushLinkList( { r: this.legSX, useSize: args.personHalfSX, max: [ args.lowerBodySX, this.legsIn ? -2: -1], min:1 } );
		...
		args.shoeSY = this.pushLinkList( this.bootsSY ? { r: this.bootsSY, useSize: args.lowerLegSY, min:1 }: { r: 0.8, useSize: args.legSX, min:1 } );

		args.legMaxSX = this.pushLinkList( { r: sideView ? 0.8: 1, useSize: args.lowerBodySX, a:-2, max: [ args.legSX,2], min: args.legSX } );

		args.thighsSX = this.pushLinkList( { useSize: args.personHalfSX, r: this.thights || 1, max: args.legMaxSX, min:1 } );
		args.calvesSX = this.pushLinkList( { useSize: args.personHalfSX, r: this.calves || 1, max: args.legMaxSX, min:1 } );
								...
								list: [
									{
										...
										list: [
											...
											// Feet
											{	sY: args.shoeSY,
												sX:!this.bareFoot && this.bootsSXBig && { r:1, a: this.bootsSXBig, max: { r: 0.5, useSize: args.personHalfSX } },
												cX: true,
												fX: true,
												tX: true,
												fY: true,
												...
												list: [
													...
													!this.bareFoot && {
														...
														sX: { r:1, a:1, max: { r: 0.15, useSize: args.personHalfSX, max: [ args.calvesSX,1] }},
														fY: true
													}
												]
											}
