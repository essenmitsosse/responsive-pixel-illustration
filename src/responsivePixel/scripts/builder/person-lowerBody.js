...
Builder.prototype.LowerBody.prototype.draw = function (args, z) {
	...
	leg = {
		...
		list: [ // Check if Skirt is so long, that it covers all the feet
	...
	if (args.calc) {
		args.crotchSY = this.pushLinkList({
			r: this.crotchSY, useSize: args.legSX, max: this.mult(0.4, args.lowerBodySY), min: 1,
		});
	}
...
Builder.prototype.Belt.prototype.draw = function (args, z) {
	...
	return {
		z: z + 115,
		sY: {
			r: this.beltSY, useSize: args.crotchSY, a: 1, max: { r: 1.4, useSize: args.legSX, max: [args.crotchSY, -1] },
		},
...
Builder.prototype.Skirt.prototype.draw = function (args) {
	...
	if (args.calc) {
		args.skirtSY = this.pushLinkList({
			r: this.skirtSY, useSize: args.lowerBodySY, min: 1, max: args.lowerBodySY,
		});
...
Builder.prototype.Leg.prototype.draw = function (args, z, rightSide, behind) {
	...
	if (args.calc) {
		args.legSX = this.pushLinkList({
			r: this.legSX, useSize: args.personHalfSX, max: [args.lowerBodySX, this.legsIn ? -2 : -1], min: 1,
		});
		...
		args.legMaxSX = this.pushLinkList({
			r: sideView ? 0.8 : 1, useSize: args.lowerBodySX, a: -2, max: [args.legSX, 2], min: args.legSX,
		});

		args.thighsSX = this.pushLinkList({
			useSize: args.personHalfSX, r: this.thights || 1, max: args.legMaxSX, min: 1,
		});
		args.calvesSX = this.pushLinkList({
			useSize: args.personHalfSX, r: this.calves || 1, max: args.legMaxSX, min: 1,
		});
