...
Builder.prototype.UpperBody = function ( args ) {
	...
	this.shirtColor = args.shirtColor = ( this.IF( 0.5) ? args.secondColor: args.clothColor ).copy({ brSet: shirtColor === 4 ? 3: shirtColor + 1, max:4 });

	// Assets
	this.arm = new this.basic.Arm( args );

	if( this.IF( 0.05) ) { this.cape = new this.basic.Cape( args ); }
	
	if( this.topless && this.IF( 0.8) ) { this.nipples = new this.basic.Nipples( args ); }
	
	if( !args.animal ) {
		if( this.IF( 0.07) ) { this.suspenders = new this.basic.Suspenders( args ); }
		if( this.IF( 0.02) ) { this.strap = new this.basic.Strap( args ); }
	}
	...
	if( this.IF( 0.1 ) ) { this.logo = new this.basic.Logo( args ); }
...
Builder.prototype.UpperBody.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		...
		args.chestSX = this.pushLinkList( { r: this.chestSX, useSize: args.upperBodySX, min:1, max: [ args.upperBodySX,1] } );
		...
		args.trapSX = this.pushLinkList( { add:[ args.chestSX, this.mult( sideView ? -2: -1, args.neckSX ) ] } );
...
Builder.prototype.Stripes = function ( args ) {
	...
	// Colors
	this.stripColor = ( this.IF( 0.5) ? args.secondColor: args.clothColor ).copy({ brSet: args.clothColor.getBr()-1 });
...
Builder.prototype.Stripes.prototype.draw = function ( args, z ) {
	...
	return ( sideView || !args.right ) && {
		...
		this.randomDots ? 
			[
				{ use: args.shirt, chance: 0.5, sX: { r: this.gap, useSize: args.upperBodySY }, sY: { r: this.strip, useSize: args.upperBodySY }, mask: true },
				{ save: args.shirt }
			]
			: this.doted && [
					{
...
Builder.prototype.Suspenders = function ( args ) {
	...
	if( this.detail ) { this.detailColor = args.clothColor.copy( { brContrast: this.IF( 0.5) ? 1: -1 } ); }
...
Builder.prototype.Suspenders.prototype.draw = function ( args, z ) {
	...
	return {
		...
		list: [
			{
				sX: { r: this.strapSX * ( sideView ? 0.5: 1 ), useSize: args.trapSX, min:1, save: args.strapSX },
				x: { r: this.strapX * ( sideView ? 0.5: 1 ), useSize: args.trapSX, max: [ args.trapSX,this.sub( args.strapSX )], min: { a:0} },
				fX: true,
				list: detail
			},
			sideView && {
				sX: { r: this.strapSX * ( sideView ? 0.5: 1 ), useSize: args.trapSX, min:1, save: args.strapSX },
				x: { r: this.strapX * ( sideView ? 0.5: 1 ), useSize: args.trapSX, max: [ args.trapSX,this.sub( args.strapSX )] },
				list: detail
			}
		]
	};
...
Builder.prototype.Collar = function ( args ) {
	...
	// Colors
	if( this.tie ) { this.tieColor = args.clothColor.copy({ brSet:0}); }
	this.shirtColor = args.shirtColor;

	// Assets
	if( this.open && this.IF( 0.3) ) { this.buttons = new this.basic.Buttons( args, this.shirtColor ); }
...
Builder.prototype.Collar.prototype.draw = function ( args, z ) {
	...
	return {
		z:10,
		color: this.shirtColor.get(),
		sX: this.scarf ? { add: [ args.headSX ], max: args.chestSX }: [ args.neckSX,sideView ? 2: 1],
...
Builder.prototype.Cleavage.prototype.draw = function ( args, z ) {
	...
	if( args.calc ) {
		args.cleavageSX = this.pushLinkList( { r: this.cleavageSX, useSize: args.neckSX, max: [ args.chestSX,-2] } );
		args.cleavageX = this.pushLinkList( 
			sideView ? 
				[ this.mult( 0.5, args.chestSX ), this.mult( -0.5, args.cleavageSX ), this.sub( args.collarSX ) ] 
				: [ args.chestSX, this.mult( -1, args.cleavageSX ) ]
		);
		
		if( this.sleeveless ) {
			if( sideView ) {
				args.cleavageRightX = this.pushLinkList( { add:[ args.chestSX, this.sub( args.cleavageX ), this.sub( args.cleavageSX ) ] } );
			}
...
Builder.prototype.Cape.prototype.draw = function ( args ) {
	...
	if( args.calc ) {
		...
		args.capeSY = this.pushLinkList( { r: this.capeSY, useSize: args.fullBodySY, max: [ args.fullBodySY,-1], min: args.capeFrontSY } );
	}
...
Builder.prototype.Strap = function ( args ) {
	...
	// Color
	this.strapColor = args.beltColor || ( args.beltColor = ( this.IF( 0.5) ? args.firstColor: args.secondColor ).copy( { brContrast: this.IF() ? 2: -2 } ) );
...
Builder.prototype.Strap.prototype.draw = function ( args, z ) {
	...
	if( args.calc ) {
		args.strapTickness = this.pushLinkList( { r: this.thickness, useSize: args.personSY, min:1 } );
	}
	...
	return ( sideView || args.right ) && {
		...
		id: "strap"+nr,
		list: [
			{ tY: true, tX: true, clear: true, sY: args.strapTickness, mX: this.sub( args.strapTickness ) },
			{
				z: z,
				...
				points: [
					...
					{ fX: true, x: { r: sideView ? 1: 2, useSize: args.personSX, add: [this.mult(-0.5, args.strapTickness ),-2] } }
				]
			}
		]
	};
