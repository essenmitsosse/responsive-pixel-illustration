...
Builder.prototype.UpperBody.prototype.draw = function (args) {
	...
	if (args.calc) {
		...
		args.chestSX = this.pushLinkList({
			r: this.chestSX, useSize: args.upperBodySX, min: 1, max: [args.upperBodySX, 1],
		});
...
Builder.prototype.Suspenders.prototype.draw = function (args, z) {
	...
	return {
		...
		list: [
			{
				sX: {
					r: this.strapSX * (sideView ? 0.5 : 1), useSize: args.trapSX, min: 1, save: args.strapSX,
				},
				x: {
					r: this.strapX * (sideView ? 0.5 : 1), useSize: args.trapSX, max: [args.trapSX, this.sub(args.strapSX)], min: { a: 0 },
				},
				fX: true,
				list: detail,
			},
...
Builder.prototype.Cape.prototype.draw = function (args) {
	...
	if (args.calc) {
		...
		args.capeSY = this.pushLinkList({
			r: this.capeSY, useSize: args.fullBodySY, max: [args.fullBodySY, -1], min: args.capeFrontSY,
		});
	}
