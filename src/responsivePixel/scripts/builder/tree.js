			...
			list: [
				...
				this.crooked ? 
					{ save: "trunk"+code, rX: this.reflectCrookedTrunk, list: [
						{ sY: args.trunkSX, fY: true, sX: { r: this.crookedSX, useSize: args.trunkSX }, list: [
							{ },
							{ fX: true, sX: args.trunkSX, sY: [ args.treeSqu, this.sub(args.topTrunk) ], fY: true, tY: true }
						] },
					]}
					: { save: "trunk"+code },
			] 
...
Builder.prototype.Tree.prototype.addBranches = function ( args, hor, parentLeft, count, level ) {
	var list = [],
		...
		strip = { stripes: { random: {r: -0.5}, strip: this.leaveX, change: {r: this.leaveChange }, seed: this.nr + count + level, cut: true } },
		code = this.code,
		leaves = [
			// Shadow of Leaves
			...
			// Top of Leaves
			{ save: "leaves"+thisZ+code, tY: true, sY: {r: 0.1, min: 1}, list: [
