...
var renderer = function( args ){
	...
	var help = helper,
		...
		basicColor = [ 
			( s[0] ? 59 : 0 ) + ( s[1] ? 59 : 0 ) + ( s[2] ? 50 : 0 ) + ( s[3] ? 35 : 0 ) + ( s[4] ? 20 : 0 ) + 32,
			( s[2] ? 59 : 0 ) + ( s[3] ? 59 : 0 ) + ( s[4] ? 50 : 0 ) + ( s[0] ? 35 : 0 ) + ( s[1] ? 20 : 0 ) + 32,
			( s[4] ? 59 : 0 ) + ( s[0] ? 59 : 0 ) + ( s[1] ? 50 : 0 ) + ( s[2] ? 35 : 0 ) + ( s[3] ? 20 : 0 ) + 32
		],
		...
		renderList = [
			{ m:"borderS", list:[
				...
				( s[1] || s[2] || s[3] || s[4] ) ? 
					{ color: s[1] ? c3 : s[2] ? c3 : s[3] ? c1 : c2, list:[
						{ weight: s[1] ? 2 : 7, points:[{},{ fY:true, fX:true }] },
						{ weight: s[1] ? 2 : 7, points:[{ fY:true },{ fX:true }] }
					] }
					: undefined,

				( s[2] && s[4] ) ?
					{ color: s[1] ? c4 : c3, m:{r:.25}, list:[
						{},
						s[3] ? 
							{ color:c2, m:{r:.2} }
							: undefined
