...
TableComic.prototype.Eye.prototype.draw = function EyeDraw(args) {
	...
	/* 0_1+*/const pupilS =		this.pupilS = 		this.pushLinkList({ r: 1, useSize: args.square, a: -1 });
	/* 0_1 */const pupilSX = 		this.pupilSX = 		this.pushLinkList({
		r: this.pupilSX_,
		useSize: pupilS,
		min: {
			r: 0.5, useSize: args.square, max: 2, min: 1,
		},
	});
		...
		list: [
			...
			// Corners
			{
				...
				list: [
					...
					this.roundOuter && {
						name: 'Dot', color: this.color[0], fY: true, fX: true,
					},
