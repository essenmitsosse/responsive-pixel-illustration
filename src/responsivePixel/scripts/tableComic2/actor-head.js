...
TableComic.prototype.Head = function ( args ) {
	...
	this.hair = this.rIf( 0.8 ) && new this.basic.Hair({
		actor: this.actor,
		color: this.actor.colors
	});

	this.hat = !args.noHat && this.rIf( 0.6 ) && new this.basic.Hat({
		actor: this.actor,
		color: this.actor.colors
	});
};
...
TableComic.prototype.Head.prototype.draw = function ( args ) {
	var side = ( args.info.body && args.info.body.side ) || ( args.info.side )|| 0,
		...
		mouthAreaSY = this.pushLinkList( { add: [ sY, { r: -1, useSize: eyeAreaSY }, -2 ], min: 1 } ),
...
TableComic.prototype.Eyes.prototype.draw = function ( args ) {
	var maxEyesSX =			this.pushLinkList( [ args.sX, -1 ] ),
		...
		square = this.pushLinkList( { add:[ { r: 0.5, useSize: maxEyesCombinedSX } ], max: eyeSY } ),
...
/*0_1+*/sY = 			this.sY = 			this.pushLinkList( { r: 1, useSize: sYNormal, min: 1 } ),
...
/*0_1+*/pupilS =		this.pupilS = 		this.pushLinkList( { r: 1, useSize: args.square, a: -1 } ),
/*0_1 */pupilSX = 		this.pupilSX = 		this.pushLinkList( { r: this.pupilSX_, useSize: pupilS, min: { r:0.5, useSize:args.square, max:2, min: 1 } } ),
/*0_1 */pupilSY = 		this.pupilSX = 		this.pushLinkList( { r: this.pupilSY_, useSize: pupilS, min: { r:0.5, useSize:args.square, max:2, min: 1 } } ),
		...
		list: [
				...
				list:[
					{
						...
						points:[
							{ y:-1, fX:true },
							{ y:-1 },
							{ y: eyeBrowOuterY, x: [ eyeLidOvershot, rotate && !this.left ? -1 : 0 ] },
							{ y: eyeBrowOuterY, x: rotate && !this.left && -1 },
							{ y: eyeBrowInnerY, fX: true, x: rotate && this.left && -1 },
							{ y: eyeBrowInnerY, fX: true, x: [ eyeLidOvershot, rotate && this.left ? -1 : 0 ] },
						]
					},
					{
						color: this.color[ 3 ],
			...
			// Corners
			{
				...
				list: [
					...
					this.roundOuter && { name:"Dot", color:this.color[ 0 ], fY:true, fX:true },
...
TableComic.prototype.Hair.prototype.draw = function HairDraw ( args ) {
	...
	return {
		...
		list:[
			...
			{
				sY: { r: 2 },
				sX: { r: 0.3, useSize: args.eyeRestSX, add:[ { r: -1, useSize: args.side, max: { a: 0 } } ] },
				fX: true,
				stripes: {
					strip: 1,
					random: { r: -0.3, useSize: this.sY }
	...
	return { 
		...
		list: [
			...
			this.rim && {
				...
				sY: 1,
				fY: true,
				sX: this.baseCap && { r:1, max:{ r: 1 }, add: [ { r: -0.5, useSize: args.side, max: { a: 0 } }, { r: 0.5, useSize: args.side, max: { a: 0 } } ]  },
