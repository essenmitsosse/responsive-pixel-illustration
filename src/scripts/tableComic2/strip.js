	...
	if (cameraFocus) {
		...
		if (cameraFocus.map) {
			var zoomToHead1 = cameraFocus
					? 1 /
						(cameraFocus.min.obj.headSY_ || cameraFocus.min.obj.sY_)
					: 1,
				zoomToHead2 = cameraFocus
					? 1 /
						(cameraFocus.max.obj.headSY_ || cameraFocus.max.obj.sY_)
					: 1;
		...
		if (cameraFocus.map) {
			...
			if (cameraFocus.obj.sizeMap) {
				...
				info.camera.zoomBaseActorSizeSX =
					info.camera.zoomBaseActorSizeSY = {
						map: "actor-size",
